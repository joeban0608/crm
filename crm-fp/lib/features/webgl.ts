...
export default class WebglFeature implements Feature {
	name = 'Canvas Feature';
	enabled = true;
	...
	async data() {
		this.#data =
			JSON.stringify(getWebGlBasics({ cache: {} })) +
			JSON.stringify(getWebGlExtensions({ cache: {} }));

		return {
			fingerprint: this.#data,
			info: {
				webgl: {
					basics: getWebGlBasics({ cache: {} }),
					extensions: getWebGlExtensions({ cache: {} })
				}
			}
		};
	}
